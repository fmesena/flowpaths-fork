...
def compute_metrics(grouped_data):
    ...
    for width_range, group in grouped_data.items():
        ...
        if group['solved_safety'] > 0:
            ...
            results[width_range]['avg_number_of_SCCs']      = sum(group['number_of_SCCs'])      / group['solved_safety']
            results[width_range]['avg_size_of_largest_SCC'] = sum(group['size_of_largest_SCC']) / group['solved_safety']
            results[width_range]['avg_size_of_SCCs']        = max(group['avg_size_of_SCCs'])    / group['solved_safety']
...
'''
    ...
    for width_range, metrics in results.items():
        ...
        SCC_info = (f"{metrics['avg_number_of_SCCs']:.1f} ({metrics['avg_size_of_largest_SCC']})"
